...
function makeEnv() {
  var globalEnv = new _env2.default();
  globalEnv.def("print", function (callback, txt) {
    if (typeof txt !== "string") txt = txt.toString();
    _index.output.setValue(_index.output.getValue() + txt);
    callback();
  });
  globalEnv.def("clear", function (callback) {
    _index.output.setValue('');
    callback();
  });
  ...
  globalEnv.def("sleep", function (callback, milliseconds) {
    setTimeout(function () {
      (0, _evaluate.Execute)(callback, [false]); // continuations expect a value, pass false
    }, milliseconds);
  });
...
var _codemirror = __webpack_require__(9);
...
var _stdenv = __webpack_require__(6);

var _stdenv2 = _interopRequireDefault(_stdenv);

var _evaluate = __webpack_require__(0);

var _mode = __webpack_require__(8);
...
_codemirror2.default.defineMode('gamma', _mode2.default);
editor = (0, _codemirror2.default)(editor, {
  ...
  mode: "gamma",
  tabSize: 2,
  extraKeys: (_extraKeys = {}, _defineProperty(_extraKeys, "Ctrl-Enter", function CtrlEnter() {
    display();
  }), _defineProperty(_extraKeys, "Ctrl-L", function CtrlL() {
    output.setValue('');
  }), _extraKeys)
});
editor.widgets = [];
var output = document.getElementById('output');
_codemirror2.default.defineMode("console", function () {
  return {
...
output = (0, _codemirror2.default)(output, {
  readOnly: "nocursor",
  theme: "railscasts",
  lineWrapping: true,
  scrollbarStyle: "overlay",
...
function display() {
  // clear errors
  editor.widgets.forEach(function (w) {
    return w.clear();
  });
  editor.widgets = [];
  ...
  code = (0, _input2.default)(code, editor);
  code = (0, _token2.default)(code);

  // define environment
  var globalEnv = (0, _stdenv2.default)();
  function print(txt) {
    if (typeof txt !== "string") txt = txt.toString();
  ...
  var x = (0, _parse2.default)(code);
  window.x = x;
  window.onerror = function (msg, source, line, col, e) {
    print(e);
    window.e = e;
  };
  (0, _evaluate.Execute)(_evaluate.cpsEvaluate, [x, globalEnv, function (result) {
    if (!(output.getValue().endsWith('\n') || output.getValue() == "")) print('\n');
    print('*** Result: ' + result + '\n');
  }]);
}
...
function mode(opt) {
  ...
  function token(stream, state) {
    ...
    function read_string(ch) {
      ...
      while (!stream.eol()) {
        var _ch = stream.next();
        var _next = stream.peek();
        if (_ch != "\\" && _next == state.string) {
          stream.next();
          state.string = false;
          state.escape = false;
          return "string";
    ...
    function read_ident() {
      stream.eatWhile(_tokencommon2.default.is_id);
      var curr = stream.current().trim();
      var cState = state.varState[state.varState.length - 1];
      if (_tokencommon2.default.is_keyword(curr)) {
    ...
    function read_op() {
      stream.eatWhile(_tokencommon2.default.is_op);
      return "operator";
    }
    stream.eatSpace();
    var ch = stream.next();
    ...
    if (_tokencommon2.default.is_digit(ch)) return read_number(ch);
    if (_tokencommon2.default.is_id_start(ch) || ch == "-" && next == ">") return read_ident();
    if (_tokencommon2.default.is_punc(ch)) return read_punc();
    if (_tokencommon2.default.is_op(ch)) return read_op();
    if (ch == "?") return "atom";
  ...
  return {
    startState: function startState() {
      return {
        ...
        context: null,
        escape: false,
        varState: [Object.keys((0, _stdenv2.default)().vars)]
      };
    },
    ...
    indent: function indent(state, text) {
      var indent = state.varState.length - 1;
